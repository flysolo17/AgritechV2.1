...
Color getNotificationColor(TransactionStatus status) {
  switch (status) {
    case TransactionStatus.PENDING:
      return const Color.fromARGB(255, 245, 231, 43);
    case TransactionStatus.OUT_OF_DELIVERY ||
          TransactionStatus.READY_TO_DELIVER ||
          TransactionStatus.READY_TO_PICK_UP:
      return const Color.fromARGB(255, 35, 80, 245);
    case TransactionStatus.COMPLETED || TransactionStatus.ACCEPTED:
      return const Color.fromARGB(255, 44, 105, 46);
    case TransactionStatus.CANCELLED || TransactionStatus.FAILED:
      return const Color.fromARGB(255, 219, 13, 13);
    default:
      return const Color.fromARGB(255, 245, 231, 43);
  }
}

IconData getIcon(TransactionStatus status) {
  switch (status) {
    case TransactionStatus.PENDING:
...
String formatTransactionTime(DateTime dateTime) {
  return DateFormat.jm().format(dateTime); // e.g., "9:40 AM"
}
